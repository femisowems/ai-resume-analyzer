-- Force replacement of the function to fix ambiguity
...
begin
  ...
  with resume_apps as (
    select 
      ...
      coalesce(rv.analysis_result->'strengths', '[]'::jsonb) as strengths,
      coalesce((rv.analysis_result->>'score')::numeric, 0) as base_score,
      ja.status,
      ja.match_score
    from resumes r
  ...
  select
    ...
    max(strengths) as top_strengths
  from resume_apps
  group by r_id, r_title, r_created_at;
end;
$$;
